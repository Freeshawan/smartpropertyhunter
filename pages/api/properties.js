...
export default async function handler(req, res) {
  const { transaction, property_type, country, city, neighborhood, min_bedrooms, max_bedrooms, has_garage, has_garden, has_terrace, min_price, max_price } = req.query;
  ...
  try {
    const { rows } = await pool.query(`SELECT * FROM properties ${where} ORDER BY price`, values);
